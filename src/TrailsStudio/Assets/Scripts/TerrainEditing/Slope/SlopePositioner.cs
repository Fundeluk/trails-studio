...
namespace TerrainEditing.Slope
{
    ...
    public class SlopePositioner : Positioner
    {
        ...
        private bool ValidatePosition(Vector3 position)
        {
            ...
            if (toHit.magnitude < SlopeSettings.MinBuildDistance)
            {
                StudioUIManager.Instance.ShowMessage($"Slope must be at least {SlopeSettings.MinBuildDistance:F2} away from the last line element.", 2f);
                return false;
            }
            else if (toHit.magnitude > SlopeSettings.MaxBuildDistance)
            {
                StudioUIManager.Instance.ShowMessage($"Slope must be at most {SlopeSettings.MaxBuildDistance:F2} away from the last line element.", 2f);
                return false;
            }
