...
namespace Obstacles.TakeOff
{
    ...
    {       
        ...
        public float GetMaxCarveAngle()
        {
            ...
            float carveAngle = Mathf.Atan2(2 * width * transitionLength, 
                Mathf.Abs(Mathf.Pow(transitionLength,2 ) - Mathf.Pow(width, 2)));
            
            carveAngle = Mathf.Clamp(carveAngle, -TakeoffSettings.MaxCarveAngleDeg * Mathf.Deg2Rad, 
                TakeoffSettings.MaxCarveAngleDeg * Mathf.Deg2Rad);
        ...
        {            
            ...
            angle = Mathf.Clamp(angle, -1f, 1f); // Ensure angle is between -1 and 1 for carving            
