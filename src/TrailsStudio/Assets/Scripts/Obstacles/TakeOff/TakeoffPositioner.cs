...
namespace Obstacles.TakeOff
{
    ...
    public class TakeoffPositioner : Positioner
    {
        ...
        /// The default value is <see cref="TakeoffSettings.MinBuildDistance"/>.<br/>
        ...
        public override void OnEnable()
        {
            ...
            builder.SetPosition(lastLineElement.GetEndPoint() + (TakeoffSettings.MinBuildDistance + builder.GetTransitionLengthXZ()) * lastLineElement.GetRideDirection());    
            ...
            {
                var message = TerrainManager.Instance.ActiveSlope == null
                    ? "Insufficient speed to exit the takeoff on this position. Try adjusting its height and radius. If that does not work, you probably have to add a slope change."
                    : "Insufficient speed to exit the takeoff on this position. Try adjusting its height and radius or move it along the slope change.";
        ...
        protected override bool TrySetPosition(Vector3 newPosition)
        {
            Vector3 lastElemEndPoint = lastLineElement.GetEndPoint();
            Vector3 rideDirection = Vector3.ProjectOnPlane(lastLineElement.GetRideDirection(), Vector3.up);
